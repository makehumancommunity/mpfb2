...
NodeWrapperMpfbValueRamp4 = dynamic_import("mpfb.entities.nodemodel.v2.composites.nodewrappermpfbvalueramp4", "NodeWrapperMpfbValueRamp4")

def test_composite_is_available():
    assert NodeWrapperMpfbValueRamp4
...
def test_composite_can_create_instance():
    ...
    node = NodeWrapperMpfbValueRamp4.create_instance(node_tree)
    assert node
    assert node.node_tree.name == "MpfbValueRamp4"
    ...
    assert "Map Range.003" in node.node_tree.nodes
    assert "Math.022" in node.node_tree.nodes
    assert "Math.023" in node.node_tree.nodes
    assert "Math.024" in node.node_tree.nodes
    assert "Math.025" in node.node_tree.nodes
    assert "Math.026" in node.node_tree.nodes
    assert "Math.027" in node.node_tree.nodes
    assert "Math.029" in node.node_tree.nodes
    assert "Math.010" in node.node_tree.nodes
    assert "Math.019" in node.node_tree.nodes
    assert "Math.028" in node.node_tree.nodes
    assert "Math.021" in node.node_tree.nodes
    assert "Math.006" in node.node_tree.nodes
    assert "Math.030" in node.node_tree.nodes
...
def test_composite_validate_tree():
    ...
    node_tree = NodeService.create_node_tree(node_tree_name)
    node = NodeWrapperMpfbValueRamp4.create_instance(node_tree)
    assert NodeWrapperMpfbValueRamp4.validate_tree_against_original_def()
    node_tree.nodes.remove(node)
    NodeService.destroy_node_tree(node_tree)
