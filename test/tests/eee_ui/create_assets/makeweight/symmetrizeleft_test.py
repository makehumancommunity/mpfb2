"""Tests for the MakeWeight SymmetrizeLeft operator."""

import bpy
from .... import dynamic_import, ObjectService
from ..._helpers import MockOperatorBase, HumanFixture, HumanWithRigFixture

MPFB_OT_SymmetrizeLeftOperator = dynamic_import("mpfb.ui.create_assets.makeweight.operators.symmetrizeleft", "MPFB_OT_SymmetrizeLeftOperator")


def test_symmetrize_left_is_registered():
    assert bpy.ops.mpfb.symmetrize_makeweight_left is not None
...
def test_symmetrize_left_poll_true_with_basemesh():
    with HumanFixture() as fixture:
        assert MPFB_OT_SymmetrizeLeftOperator.poll(bpy.context)


def test_symmetrize_left_execute_errors_without_rig():
    with HumanFixture() as fixture:
        mockself = MockOperatorBase()
        result = MPFB_OT_SymmetrizeLeftOperator.hardened_execute(mockself, bpy.context)
        assert result == {'FINISHED'}
        mockself.mock_report.assert_reported('ERROR', "Could not find a rig")


def test_symmetrize_left_execute_with_rig():
