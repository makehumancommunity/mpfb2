...
_LOG = LogService.get_logger("armik.armik")
...
class ArmIk():
    ...
    def __init__(self, which_arm, settings):
        _LOG.debug("Constructing ArmIk object")
        self.which_arm = which_arm
        self.settings = dict()
        self._bone_info = dict()

        for key_sided in settings.keys():
            key = str(key_sided).replace(self.which_arm + "_", "")
            self.settings[key] = settings[key_sided]
    ...
    def apply_ik(self, armature_object):
        ...
        if self.settings["arm_ik_type"] == "LOWERUPPER":
            self._apply_lower_upper(armature_object)

        if self.settings["arm_ik_type"] == "LOWERUPPERSHOULDER":
            self._apply_lower_upper_shoulder(armature_object)

        if self.settings["arm_ik_type"] == "ARMWITHPOLE":
            self._apply_arm_with_pole(armature_object)

        if self.settings["arm_ik_type"] == "ARMANDSHOULERWITHPOLE":
            self._remove_arm_and_shoulder_with_pole(armature_object)

        if self.settings["arm_ik_type"] == "ARMCHAIN":
            self._apply_arm_chain(armature_object)

        if self.settings["arm_ik_type"] == "SHOULDERCHAIN":
    ...
    def remove_ik(self, armature_object):
        _LOG.enter()
        self._bone_info = RigService.get_bone_orientation_info_as_dict(armature_object)
        mode = str(IkFkProperties.get_value(self.which_arm + "_arm_mode", entity_reference=armature_object)).strip()
    ...
    def get_instance(which_arm, settings, rigtype="Default"):
        _LOG.enter()
        if rigtype == "Default":
            from mpfb.entities.armik.defaultarmik import DefaultArmIk  # pylint: disable=C0415
            return DefaultArmIk(which_arm, settings)
    ...
    def _reset_bones(self, bone_names, armature_object):
        _LOG.debug("preserve ik", self.settings["arm_preserve_ik"])
