...
_LOG = LogService.get_logger("legik.legik")
...
class LegIk():
    ...
    def __init__(self, which_leg, settings):
        _LOG.debug("Constructing LegIk object")
        self.which_leg = which_leg
        self.settings = dict()
        self._bone_info = dict()

        for key_sided in settings.keys():
            key = str(key_sided).replace(self.which_leg + "_", "")
    ...
    def apply_ik(self, armature_object):
        ...
        if self.settings["leg_ik_type"] == "LOWERUPPER":
            self._apply_lower_upper(armature_object)

        if self.settings["leg_ik_type"] == "LOWERUPPERHIP":
    ...
    def remove_ik(self, armature_object):
        _LOG.enter()
        self._bone_info = RigService.get_bone_orientation_info_as_dict(armature_object)
        mode = str(IkFkProperties.get_value(self.which_leg + "_leg_mode", entity_reference=armature_object)).strip()
    ...
    def get_instance(which_leg, settings, rigtype="Default"):
        _LOG.enter()
        if rigtype == "Default":
            from mpfb.entities.legik.defaultlegik import DefaultLegIk  # pylint: disable=C0415
            return DefaultLegIk(which_leg, settings)
        return LegIk(which_leg, settings)
