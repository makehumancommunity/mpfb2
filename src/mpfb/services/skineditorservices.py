...
# Description:  Usefull functions for skin editor
# ------------------------------------------------------------------------------

import os, bpy, json, time

class SkinEditorService():
    @staticmethod
...
class _SkinEditorService():
    ...
    def load_textures(self, textures_json_path):
        """Load textures from paths in a JSON file from given folder"""
    ...
    def create_texture_nodes(self, material, textures, location_x, location_y, mix_node_offset):
        """Creates texture nodes and mix nodes for given textures when creating new material."""
    ...
    def add_textures_to_scene(self, scene, albedo_textures, normal_textures):
        """Adds drawers, albedo and normal textures into scene properties so they can be edited through UI"""
    ...
    # UNUSED cannot write into scene while drawing
    ...
    def add_multiply_color_property(self, scene, mix_node):
        """Adds property for skin color RGB Node color picker"""
    ...
    def add_bake_property(self, scene):
        """Adds properties for Freckles"""
        actual_dir = os.path.dirname(__file__)
        default_dest = os.path.join(actual_dir, "../data/user_export/")
    ...
    def bake_category(self, context, texture_categories, categories_to_bake, output_name, texture_resolution, baking_type, base_alpha=False):
        ...
        #NOTE Base alpha means base category has alredy been baked and base texture is swapped for transparent texture for other categories to bake withput background
        scene = context.scene
        obj = context.object
        if not obj or not obj.active_material:
            print("No active material found.")
        ...
        if baking_type != "normal" and has_diffuse:
            if(not base_alpha):
                # Baking base with standard bake interface
                ...
                # Bake other textures using compositor
        ...
        if baking_type != "diffuse" and has_normal:
            if(not base_alpha):
                # Bake normal base with baking interface
                image = bpy.data.images.load(texture_path)
                texture_node = nodes.new(type="ShaderNodeTexImage")
                texture_node.image = image
                ...
                # Bake rest of normal categories with compositor
                # Save baked texture destinations
                save_path = os.path.join(dest_dir, f"{output_name}_norm.png")
                scene.render.filepath = save_path
    ...
    def connect_baked_textures_to_shader(self, context, texture_categories):
        # Connect freshly baked textures to shader instead of modular texture setup
        scene = context.scene
        obj = context.object
        if not obj or not obj.active_material:
...
_SKINEDITOR_SERVICE = _SkinEditorService()