...
class MPFB_OT_Save_Rig_To_Library_Operator(MpfbOperator):
    ...
    def hardened_execute(self, context):
        ...
        # Validate rig name
        if not ctx.library_rig_name or not re.match(r'^[a-zA-Z0-9_]+$', ctx.library_rig_name):
        ...
        identifying_bones = [b.strip() for b in ctx.library_identifying_bones.split(",") if b.strip()]
        ...
        rig_file = os.path.join(rigs_dir, ctx.library_rig_name + ".json")
        ...
        if ctx.library_also_save_weights:
            ...
            if basemesh is None:
                ...
                weights_file = os.path.join(rigs_dir, "weights." + ctx.library_rig_name + ".json")
