...
class MPFB_OT_Setup_Sculpt_Operator(MpfbOperator):
    ...
    def _create_clean_copies(self, context, obj, apply_armature, delete_helpers, remove_delete, create_source_copy=False):
        ...
        _LOG.debug("context.active_object", context.active_object)
    ...
    def hardened_execute(self, context):
        ...
        if ctx.sculpt_strategy in ["DESTCOPY", "SOURCEDESTCOPY"]:

            source_copy = ctx.sculpt_strategy == "SOURCEDESTCOPY"
            _LOG.debug("source_copy", source_copy)
            (source, dest) = self._create_clean_copies(context, obj, ctx.apply_armature, ctx.delete_helpers, ctx.remove_delete, create_source_copy=source_copy)
            self._setup_materials(context, dest, ctx.normal_material, ctx.resolution)

            if not source:
                source = obj
                self._handle_bm(context, source, ctx.delete_helpers)
                self._handle_armature(context, source, ctx.apply_armature)

                if ctx.remove_delete:
            ...
            self._setup_multires(context, source, ctx.setup_multires, ctx.subdivisions, ctx.multires_first)
            self._setup_multires(context, dest, ctx.setup_multires, ctx.subdivisions, ctx.multires_first)
            ...
            if source_copy and ctx.hide_origin:
                parent = obj
                if obj.parent:
                    parent = obj.parent
            ...
            _LOG.debug("source_copy, obj, hide_related", (source_copy, obj, ctx.hide_related))

            if not source_copy and ctx.hide_related:
                parent = obj
                if obj.parent:
                    parent = obj.parent
        ...
        if ctx.sculpt_strategy == "ORIGIN":
            ...
            self._handle_bm(context, obj, ctx.delete_helpers)
            self._handle_armature(context, obj, ctx.apply_armature)
            ...
            self._setup_multires(context, obj, ctx.setup_multires, ctx.subdivisions, ctx.multires_first)

            if obj.parent:
                obj.location = parent.location.copy()
                obj.parent = None

        if ctx.sculpt_strategy != "ORIGIN" and ctx.adjust_settings:
            scene = context.scene
        ...
        if ctx.sculpt_strategy == "DESTCOPY" and source and MaterialService.has_materials(source):
