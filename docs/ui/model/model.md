...
Custom and user targets are named after their files, so a sufficiently long file name would produce a
Blender property identifier that is too long to register. Such targets are skipped at startup and a
warning naming the offending file is written to the MPFB log; see *Dynamic properties — detail target
sliders* below.
...
Property names are generated by `_category_identifier()`, which wraps `UiService.as_valid_identifier()` so
that the registration loop and the panel drawing code always derive the same string:
...
Blender rejects property identifiers of 64 characters or more, and since these properties are attached
while the module is being imported, a single overly long name would abort the registration of the whole
addon. Before any property is registered, `_drop_categories_with_unusable_identifiers()` measures every
category against `UiService.MAX_IDENTIFIER_LENGTH` (63) and removes those that would overflow, so they
are neither registered nor drawn. Each removal is logged as a warning naming the full path of the file,
so the user can rename it.

The section name and the separator count toward the limit, and sided categories additionally carry an
`l-` or `r-` prefix, leaving them two characters less budget than unsided ones. A category is measured
only against the identifiers it actually registers, so an unsided category is not penalised for a sided
variant it never creates.

No built-in target comes anywhere near the limit — the longest identifier in `target.json` is 43
characters — so in practice this only ever affects custom and user targets. Macro/phenotype targets are
also unaffected: they are not part of these sections at all, and are named through a fixed
`mpfb_macropanel_` prefix and, on the shape key side, through `TargetService.encode_shapekey_name()`
against the separate 61-character shape key limit.
