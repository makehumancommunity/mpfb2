...
The New Human section lives in `src/mpfb/ui/new_human/`. Its root panel is `MPFB_PT_New_Panel`, which appears as "New human" under the Model category in Blender's sidebar. This panel acts as a container: four child panels are nested inside it, each offering a different workflow for creating a human character.
...
| `randomize/` | "Random human" | Create a character with a randomized phenotype |
...
- [randomize.md](randomize.md) — "Random human" panel
